...
export default function Header() {
  ...
  // Seule la page d'accueil a un hero sombre sous le header : partout
  // ailleurs il est opaque d'emblée, et une cale de sa hauteur évite qu'il
  // recouvre le titre de la page.
  const home = pathname === "/";
  const solid = scrolled || open || !home;

  return (
    <>
      {home ? null : <div aria-hidden className="h-19 md:h-22" />}
      <header
          ...
          <ul className="container-latour flex flex-col py-3">
            {nav.map((item) => (
              ...
              >
                <Link
                  href={item.href}
                  className="m3-menu-item m3-state text-ink"
                >
                  {item.label}
                </Link>
            ...
            <li className="pb-6 text-center">
              <a
                href={site.phoneHref}
                className="m3-button m3-text-button m3-state"
              >
                {site.phone}
              </a>
            </li>
